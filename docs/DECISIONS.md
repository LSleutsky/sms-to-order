...
### 22. An empty extraction is a failure, and extraction can be re-run

Mine. On the final wiped-volume Docker run, fixture 09, three plain order lines, came back from the model with zero lines and one note that said "...". The code stored that as a successful extraction, the queue showed it as needing review with nothing to review, and retry only applied to unparsed messages, so the only way out was to send the text again. Two changes. Zero lines with no notes, or notes with no words in them, now lands as unparsed with the reason `empty_output`, so it shows up red with a retry button. And extraction can be re-run on any message that has not been accepted, from a button under the raw text, since a wrong extraction is not only an empty one. A text that is not an order still comes out as zero lines with its notes, like fixture 08, and that is still fine.

---

## How I built it

I wrote the plan and the tool's rules before any code: `docs/BUILD-PLAN.md`, `CLAUDE.md`, and one slash command per phase under `.claude/commands/`. Then I ran the build through Claude Code one phase at a time. I wrote the ten fixtures by hand before any parser existed, so the parser had to meet the texts and not the other way around. At each stop the tool printed what it had and waited: the catalog tokens and column roles, the extraction schema, raw text next to extracted lines for every fixture, the scores table, the highlight question. I read the output and decided. I made every commit myself after reading the diff.

What I wrote or decided by hand: the build plan, the tool rules in `CLAUDE.md`, the six phase commands, every fixture, the feature and stack choices in entries 1 through 5, the call to load the catalog as-is and to index part numbers out of the descriptions, notes as a list, the failure states, the top-three rule, no span highlighting, no accessibility pass, the send box, and the order view. The wording of the entries in this log started as the tool's draft of what I said at each stop, and I rewrote what did not sound like me.

What the tool produced under those rules: the pnpm workspace, `Dockerfile`, `compose.yaml`, and the lint setup; on the server, `config.ts`, `db.ts`, `index.ts`, `catalog.ts` with the token rules, `extraction.ts` with the prompt and schema, `messages.ts`, `matcher.ts` with the normalization and scoring, `matching.ts`, `orders.ts`, `fixtures.ts`, `seed.ts`, and the three routers; on the client, every component and `api.ts`; `docs/FIXTURE-SCORES.md` from the seeded database; and the first drafts of `README.md` and `docs/OUT-OF-SCOPE.md`.

What I checked and what I caught: at the catalog stop I read ten rows of tokens and took the tool's proposal to drop the false ones and add plain-digit numbers, entry 8. At the extraction stop I read all ten fixtures raw next to extracted and found nothing to change, entry 13, so that phase has one commit where I had planned two. At the matching stop I read the scores table and took the 0.85 cutoff after checking every top candidate above it was right and the best wrong candidate anywhere sat at 0.83, entry 15. After the UI phase I clicked the fixtures through and found two things the plan had not covered: there was no way to send a text without curl, and an accepted message showed nothing about the order it produced, entries 20 and 21. A last pass over every path before the wrap caught one bug, when extraction failed the inbound response reported the reason as `pending` because the code returned a row it had read before the update, and the wiped-volume Docker run caught the empty extraction in entry 22.
